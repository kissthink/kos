...
void TcpManager::receive(IpAddress from, uint16_t sourcePort, uint16_t destPort, Tcp::tcpHeader* header, uintptr_t payload, size_t payloadSize, Network* pCard)
{
  ...
  if((stateBlock = m_StateBlocks.lookup(handle)) == 0)
  {
    ...
    if((stateBlock = m_ListeningStateBlocks.lookup(handle)) == 0)
    {
      ...
      DBG::outln(DBG::Warning, "TCP Packet arriving on port ", handle.localPort, " has no destination.");
...
#if TCP_DEBUG
  DBG::outln(DBG::Net, "TCP Packet arrived while stateBlock in ", Tcp::stateString(stateBlock->currentState), " [remote port = ", stateBlock->remoteHost.remotePort, "] [connId = ", stateBlock->connId, "].");

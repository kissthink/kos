...
void Icmpv6::receive(IpAddress from, IpAddress to, uintptr_t packet, size_t nBytes, IpBase *pIp, Network* pCard)
{
  ...
  if(checksum) {
    DBG::outln(DBG::Warning, "ICMPv6: checksum incorrect on incoming packet from ", from.toString());
    pCard->badPacket();
    return;
  }
...
void Icmpv6::send(IpAddress dest, IpAddress from, uint8_t type, uint8_t code, uintptr_t payload, size_t nBytes, Network *pCard)
{
  if(dest.isMulticast() && !pCard) {
    DBG::outln(DBG::Warning, "ICMPv6: Packet had a multicast destination, but no given network interface. Can't figure out where to send this!");
    return;
  }
  ...
  if(dest.isUnicast() && !dest.isLinkLocal()) {
    ...
    if(!pCard) {
      DBG::outln(DBG::Warning, "ICMPv6: Couldn't find a route for destination '", dest.toString(), "'.");
      return;
    }
  }

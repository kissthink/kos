...
bool TcpEndpoint::connect(Endpoint::RemoteEndpoint remoteHost, bool bBlock)
{
  ...
  if (m_ConnId == 0)
    DBG::outln(DBG::Warning, "TcpEndpoint::connect: got 0 for the connection id");
...
{
  if (nBytes > 0xFFFF) {
    DBG::outln(DBG::Warning, "Dud length passed to depositPayload!");
    return 0;
  }
  ...
  if (push && m_ShadowDataStream.getDataSize()) {
    ...
    if(sz > m_ShadowDataStream.getSize()) {
      DBG::outln(DBG::Error, "TCP: Shadow data stream has potentially been corrupted: ", sz, ", ", m_ShadowDataStream.getSize(), "!");
      return 0;
    }
    ...
    if(!o)
      DBG::outln(DBG::Net, "TCP: wrote zero bytes to a data stream!");
...
{
  if (block) {
#if 0 // do I really need TimeoutGuard??
...
bool TcpEndpoint::shutdown(ShutdownType what)
{
  if(what == ShutSending) {
    ...
    DBG::outln(DBG::Net, "TCP: Endpoint ", FmtHex(reinterpret_cast<uintptr_t>(this)), ": no longer able to send!");
    TcpManager::instance().Shutdown(m_ConnId);
    return true;
  } else if(what == ShutReceiving) {
    DBG::outln(DBG::Net, "TCP: Endpoint ", FmtHex(reinterpret_cast<uintptr_t>(this)), ": no longer able to receive!");

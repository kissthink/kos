...
Cache::~Cache()
{
  ...
  for(Tree<uintptr_t, CachePage*>::Iterator it = m_Pages.begin();
    ...
    if (PageManager::isMapped(vaddr(page->location))) {
      kernelSpace.releasePages<1>(vaddr(page->location), pagesize<1>());
//            Processor::getFrameManager()->release(PageManager::vtol(vaddr(page->location)), pagesize<1>());
#if 0
...
void Cache::compact()
{
  ...
  while(m_PagesToErase.count()) {
    ...
    if (PageManager::isMapped(vaddr(page))) {
//            Processor::getFrameManager()->release(PageManager::vtol(vaddr(page)), pagesize<1>());

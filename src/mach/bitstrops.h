...
static inline mword find_lsb_cond( mword x, mword alt ) {
...
static inline mword pow2align_up( mword x ) {
  return pow2(ceilinglog2(x));
}

static inline mword pow2align_down( mword x ) {
  return pow2(floorlog2(x));
}

static inline constexpr mword builtin_clog2( mword x ) {
  return mwordbits - __builtin_clzll(x);
}

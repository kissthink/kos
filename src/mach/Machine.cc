...
//  loadTR(tssSelector << 3); // TODO: separate cache-aligned TSS per core?
  clearLDT();
...
void Machine::parseMBI( vaddr mbi, vaddr& rsdp ) {
  KASSERT( elf_version ( EV_CURRENT ) != EV_NONE, "elf library" );
  KASSERT( !(mbi & 7), mbi );
  ...
  if (tag->type == MULTIBOOT_TAG_TYPE_END || mbi >= mbiEnd) break;
    ...
    case MULTIBOOT_TAG_TYPE_MODULE: {
      ...
      switch (ek) {
        ...
        case ELF_K_NONE: DBG::out(DBG::Boot, " data"); break;
...
inline void Machine::setupGDT(unsigned int number, unsigned int dpl, uint32_t address, bool code) {
...
inline void Machine::setupTSS(unsigned int number, uint64_t tssAddress) {
  SystemDescriptor* tssDesc = (SystemDescriptor*)&gdt[number];
  tssDesc->Base00 = (tssAddress & 0x000000000000FFFF);
  tssDesc->Base16 = (tssAddress & 0x0000000000FF0000) >> 16;
  tssDesc->Type = 0x9;
  tssDesc->DPL = 3;
  tssDesc->P = 1;
  tssDesc->Base24 = (tssAddress & 0x00000000FF000000) >> 24;
  tssDesc->Base32 = (tssAddress & 0xFFFFFFFF00000000) >> 32;
}

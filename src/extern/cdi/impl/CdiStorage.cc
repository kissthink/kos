...
static cdi_storage_device* masterDevice = nullptr;
...
//  cdi_storage_driver* driver = (cdi_storage_driver *) device->dev.driver;
  cdi_storage_driver* driver = (cdi_storage_driver *) masterDevice->dev.driver;
  size_t block_size = masterDevice->block_size;
  ...
  if (((pos % block_size) == 0) && (((pos + size) % block_size) == 0)) {
    return driver->read_blocks(masterDevice, block_read_start, block_read_count, dest);
  } else {
    block_read_count += 1;
    uint8_t* buffer = new uint8_t[block_read_count * block_size];
    if (driver->read_blocks(masterDevice, block_read_start, block_read_count, buffer) != 0) return -1;
...
//  cdi_storage_driver* driver = (cdi_storage_driver *) device->dev.driver;
  ...
  if (offset != 0) {
    ...
    if (driver->read_blocks(masterDevice, block_write_start, 1, buffer) != 0) return -1;
    memcpy(buffer + offset, src, tmp_size);
    if (driver->write_blocks(masterDevice, block_write_start, 1, buffer) != 0) return -1;
    size -= tmp_size;
  ...
  if (tmp_size != 0) {
    if (driver->write_blocks(masterDevice, block_write_start, tmp_size, src) != 0) return -1;

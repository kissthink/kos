...
*/
#include "drivers/common/cdi/CdiDisk.h"
#include "kern/ServiceManager.h"
#include "vfs/VFS.h"
...
bool CdiDisk::initialise()
{
    ...
    ServiceFeatures *pFeatures = ServiceManager::enumerateOperations(String("partition"));
    Service         *pService  = ServiceManager::getService(String("partition"));
    DBG::outln(DBG::CDI, "Asking if the partition provider supports touch");
    if(pFeatures->provides(ServiceFeatures::touch))
    {
        DBG::outln(DBG::CDI, "It does, attempting to inform the partitioner of our presence...");
        if(pService)
        {
            ...
            {
                DBG::outln(DBG::CDI, "Successful.");
            }
            else
            {
                DBG::outln(DBG::CDI, "Failed.");
                return false;
            }
        }
        else
        {
            DBG::outln(DBG::Error, "FileDisk: Couldn't tell the partition service about the new disk presence");
            return false;
        }
    }
    else
    {
        DBG::outln(DBG::Error, "FileDisk: Partition service doesn't appear to support touch");
        return false;
    }
...
{
    KASSERT1((location % 512) == 0, (location % 512));

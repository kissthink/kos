...
static const int NUM64BITREGS = 17;

/* Number of 32 bit registers. */
// EFLAGS, CS, SS, DS, ES, FS, GS
static const int NUM32BITREGS = 7;
...
//asm("       movw registers_32+8, %ss");
asm("       movw registers_32+12,   %ds");
asm("       movw registers_32+16,   %es");
//asm("       movw registers_32+20,   %fs");            // WARNING: this line causes pagefault
                                                        // on address fs:4
asm("       movw registers_32+24,   %gs");
...
#define SAVE_REGISTERS1() \
  ...
  /* r8-r15 */                                             \
  asm ("movq %r8, registers_64+64");       /* 64 */        \
  asm ("movq %r9, registers_64+72");       /* 64 */        \
  asm ("movq %r10, registers_64+80");      /* 64 */        \
  asm ("movq %r11, registers_64+88");      /* 64 */        \
  asm ("movq %r12, registers_64+96");      /* 64 */        \
  asm ("movq %r13, registers_64+104");     /* 64 */        \
  asm ("movq %r14, registers_64+112");     /* 64 */        \
  asm ("movq %r15, registers_64+120");                     \
  asm ("movl $0, %eax");                                \
  /* rip (pc) eflags (ps), cs, ss saved later */        \
  /* ds */                                              \
  asm ("movw %ds, registers_32+12");    /* 32 */        \
  asm ("movw %ax, registers_32+14");                    \
  /* es */                                              \
  asm ("movw %es, registers_32+16");    /* 32 */        \
  asm ("movw %ax, registers_32+18");                    \
  /* fs */                                              \
  asm ("movw %fs, registers_32+20");    /* 32 */        \
  asm ("movw %ax, registers_32+22");                    \
  /* gs */                                              \
  asm ("movw %gs, registers_32+24");    /* 32 */        \
...
#define SAVE_REGISTERS2() \
  /* old rip */                                         \
  asm ("popq %rbx");                                    \
  asm ("movq %rbx, registers_64+128");     /* 64 */     \
  /* old cs */                                          \
  asm ("popq %rbx");                                    \
  asm ("movl %ebx, registers_32+4");                    \
  /* old eflags */                                      \
  asm ("popq %rbx");                                    \
  asm ("movl %ebx, registers_32");            /* 32 */  \
...
handle_exception (int64_t exceptionVector)
{
    ...
    *ptr++ = hexchars[regs64::RSP]; 
    *ptr++ = ':';
    ptr = mem2hex((char *)&registers_64[regs64::RSP], ptr, 8, 0);  /* SP */
    *ptr++ = ';';

    *ptr++ = hexchars[regs64::RBP]; 
    *ptr++ = ':';
    ptr = mem2hex((char *)&registers_64[regs64::RBP], ptr, 8, 0);  /* FP */
    *ptr++ = ';';

    *ptr++ = hexchars[regs64::RIP]; 
    *ptr++ = ':';
    ptr = mem2hex((char *)&registers_64[regs64::RIP], ptr, 8, 0);  /* RIP */
    *ptr++ = ';';
            ...
            case 'q':   // qOffsets, qSupported, qSymbol::
                {
                    if (strncmp(ptr, "Supported", strlen("Supported")) == 0) {
                        strcpy(remcomOutBuffer, "PacketSize=3E8");  // max packet size = 1000 bytes
                        break;
                    } else if (*ptr == 'C') {
                        remcomOutBuffer[0] = 0;     // empty response, let threadID = NULL
                        break;
                    } else if (strncmp(ptr, "Attached", strlen("Attached")) == 0) {
                        remcomOutBuffer[0] = 0;     // empty response, NOT on documentation
                        break;
                        ...
                        remcomOutBuffer[0] = 0;             // empty response works instead
                        break;
                    }
                    strcpy(remcomOutBuffer, "qHELLO");       // catches unimplemented commands
                }
                break;
            case 'H':   // Hc, Hg
                {
                    if (strncmp(ptr, "c-1", strlen("c-1")) == 0) {
                        strcpy(remcomOutBuffer, "OK");      // allow step/continue to work on all threads
                        break;
                    } else if (strncmp(ptr, "c0", strlen("c0")) == 0) {
                        strcpy(remcomOutBuffer, "OK");
                        break;
                    } else if (strncmp(ptr, "g-1", strlen("g-1")) == 0) {
                        strcpy(remcomOutBuffer, "OK");      // all future opreations apply on all threads
                        break;
                    } else if (strncmp(ptr, "g0", strlen("g0")) == 0) {
                        strcpy(remcomOutBuffer, "OK");      // no documentation
                        break;
                    }
                    strcpy(remcomOutBuffer, "HHELLO");      // catches unimplmented commands
                }
                break;
            case '?':
                remcomOutBuffer[0] = 'S';
...
//                strcpy (remcomOutBuffer, "E01");
                strcpy(remcomOutBuffer, "OK");      // force orig_rax write to pass
            }
                break;

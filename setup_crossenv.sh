...
function build() {
	cd $TMPDIR/$1-build
	make -j 4 -l 0.9 all || error "$1 build failed"
}
...
function build_gcc() {
	...
	make -j 4 -l 0.9 all || error "$GCC first pass failed"
	...
	make -C newlib clean
	make -C newlib -j 4 -l 0.9 all || error "newlib recompile failed"
	...
	make -C libstdc++-v3 -j 4 -l 0.9 all || error "libstdc++-v3 recompile failed"
...
function build_grub() {
	unpack $GRUB tar.xz
	prebuild $GRUB
	../$GRUB/configure --target=$TARGET --prefix=$CROSSDIR\
